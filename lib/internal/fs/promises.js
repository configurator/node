...
async function readFileHandle(filehandle, options) {
  ...
  if (size === 0) {
    ...
    result = Buffer.allocUnsafeSlow(size);
  }

  let endOfFile = false;
  let totalBytesRead = 0;
  do {
    let buf, index, chunkSize;
    if (size === 0) {
      buf = Buffer.allocUnsafeSlow(kReadFileMaxChunkSize);
      index = 0;
      chunkSize = kReadFileMaxChunkSize;
    } else {
      buf = result;
      index = totalBytesRead;
      chunkSize = MathMin(size - totalBytesRead, kReadFileMaxChunkSize);
    }

    const { bytesRead, buffer } =
      await read(filehandle, buf, index, chunkSize, -1);
    endOfFile = bytesRead === 0;
    if (size === 0 && bytesRead > 0) {
      chunks.push(buffer.slice(0, bytesRead));
    }
    totalBytesRead += bytesRead;
  } while (!endOfFile && totalBytesRead !== size);

  if (size === 0) {
    result = Buffer.concat(chunks, totalBytesRead);
  } else {
    result = result.slice(0, totalBytesRead);
  }
